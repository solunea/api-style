...
- "prompt": a long, extremely precise English prompt (at least 250 words) that reproduces this exact visual style for any subject with a background. CRITICAL RULES: (1) NEVER use vague filler words like "beautiful", "stunning", "amazing", "high quality" — every word must carry specific visual information. (2) The subject and its environment must fill the ENTIRE frame — NO canvas border, NO frame, NO decorative overlay, NO vignette, NO empty margin. The composition bleeds to all four edges. (3) Start with the rendering technique and medium, then describe the subject integration, then layer in lighting, color behavior, texture, and atmosphere. (4) Describe the EXACT stroke/render behavior — e.g. instead of "watercolor style", say "wet-on-wet watercolor with pigment blooming at edges, granulation visible in ultramarine washes, paper white preserved as highlights, soft cauliflower backruns in shadow areas". (5) For lighting, specify direction, quality, color temperature, falloff, and surface interaction. (6) For color, describe hue shifts across light-to-shadow transitions, edge color behavior, atmospheric color perspective. (7) Include {{subject}}, {{primary_color}}, {{accent_color}}, and optionally {{mood}}, {{lighting}}. (8) End with a comma-separated list of 20+ reinforcement tags SPECIFIC to this style (not generic — e.g. "visible brushstrokes, impasto texture, limited palette, graphic novel shading, linework weight variation, muted earth tones" rather than "8k, detailed"). Always include "full bleed, no border, no frame, no canvas" in the tags.
